...
- [שלב ה – יצירת ממשק גרפי לעבודה מול בסיס הנתונים](#שלב-ה--יצירת-ממשק-גרפי-לעבודה-מול-בסיס-הנתונים)
  - [הוראות כניסה והפעלה למערכת ניהול טורנירים](#הוראות-כניסה-והפעלה-למערכת-ניהול-טורנירים)
  - [כלים ודרך העבודה ליצירת האפליקציה](#כלים-ודרך-העבודה-ליצירת-האפליקציה)
  - [תמונות](#תמונות)
...
### תוכנית שניה
התוכנית הראשית מבצעת שני שלבים עיקריים. תחילה היא עוברת על כל הנבחרות (nationalteam) ומחשבת דירוג חדש עבור כל אחת מהן על בסיס הדירוג הקיים פחות 10, תוך ווידוא שהדירוג החדש לא יהיה שלילי (באמצעות GREATEST). אם הדירוג המוצע עומד בתנאים, היא מזמנת את הפרוצדורה update_team_ranking_if_needed לעדכון הדירוג בפועל. לאחר מכן, התוכנית עוברת על כל השחקנים בטבלת player, ומציגה את רשימת המשחקים שבהם השתתף כל שחקן באמצעות קריאה לפונקציה get_games_by_player, שבוחרת את המשחקים ומחזירה אותם כ־REFCURSOR. הפלט מוצג באמצעות RAISE NOTICE
<br>
![צילום מסך 2025-06-04 104515](https://github.com/user-attachments/assets/e4671f74-6050-4960-9e69-aa5b996ed492)
<br>
הפונקציה get_games_by_player(player_id_input INT) מקבלת מזהה שחקן ומחזירה מצביע (REFCURSOR) לטבלה זמנית שמכילה את כל המשחקים שבהם השחקן השתתף. היא משלבת שתי שאילתות SELECT עם UNION: הראשונה מחפשת את כל משחקי הכדורגל שבהם שיחק השחקן דרך טבלת playersinmatches, והשנייה מחזירה את משחקי הכדורסל של השחקן מתוך טבלת basketballplayer. לכל רשומה מתווסף גם סוג הספורט (sporttype) כדי לזהות את מקור המשחק. הפונקציה פותחת את הקורסור ומחזירה אותו לקריאה חיצונית, כך שניתן לעבור על הנתונים באופן איטרטיבי.
<br>
![צילום מסך 2025-05-28 190318](https://github.com/user-attachments/assets/2896444f-fcac-4ffb-9840-95e3b30e1705)
<br>
הפרוצדורה update_team_ranking_if_needed(team_id INT, new_rank INT, threshold NUMERIC) נועדה לעדכן את הדירוג (ranking) של נבחרת מסוימת רק אם היא עומדת בתנאי סף מסוים. תחילה היא מחשבת את ממוצע הנקודות או הגולים של כל השחקנים המשויכים לנבחרת לפי סוג הספורט – כדורגל או כדורסל. אם הממוצע שחושב גבוה או שווה ל־threshold, הדירוג בטבלת nationalteam מעודכן לערך new_rank. אחרת, לא מתבצע שינוי. בנוסף, הפרוצדורה מציגה הודעת NOTICE עם המידע לצורך מעקב.
<br>
![צילום מסך 2025-05-28 190358](https://github.com/user-attachments/assets/65c3a10a-1891-4d6c-8417-c2909196ec39)
<br>
הערת שוליים על 2 התוכניות- שמתי 2 קבצים עם חלק מההדפסה שהתוכנית עשתה
<br>

---
## שלב ה'- יצירת ממשק גרפי לעבודה מול בסיס הנתונים
### הוראות כניסה והפעלה למערכת ניהול טורנירים

מבנה התיקייה:

קובץ ראשי להפעלת הממשק – main_menu.py
הגדרות התחברות למסד הנתונים – db_config.py
ניהול טבלת שחקנים – player_crud.py
ניהול טבלת קבוצות – team_crud.py
ניהול משחקים – game_crud.py
מסך הפעלת שאילתות ופונקציות – queries_and_procedures.py


הגדרות התחברות למסד הנתונים (בקובץ `db_config.py`):
...
הפעלה:
1. הפעלת Docker.
2. ודא שהקונטיינרים פעילים (`my_postgres` ו־`pgadmin`).
3. להריץ את הקובץ הראשי מהמסוף או IDE: `python main_menu.py`
   
פונקציונליות:

מסך לניהול שחקנים: הצגה, הוספה, עדכון ומחיקה – Manage Players
ניהול קבוצות לפי פרטים כמו שם, מדינה, צבע מדים, דירוג ועוד – Manage Teams
ניהול משחקים כולל תאריך, מיקום, מזהה טורניר – Manage Games
מסך להפעלת שאילתות, פונקציות ופרוצדורות – Queries & Procedures

* שאילתות:
    * ספירת שחקנים לפי קבוצה.
    * רשימת שחקנים לפי תפקיד ושם משפחה.
* פונקציות:
   מחזירה את סך הנקודות לשחקנים – get_all_players_points()
* פרוצדורות:
   מעלה את ניסיון המאמנים – increase_experience_of_coaches()

הערות:
* כדי לראות שינויים מיידיים יש ללחוץ על כפתור Refresh במסכי הטפסים.

---
### כלים ודרך העבודה ליצירת האפליקציה
כדי לבנות את מערכת ניהול הטורנירים, שילבתי מגוון כלים מודרניים לפיתוח ממשק משתמש, ניהול מסד נתונים, והרצה בסביבה מקומית. חילקתי את המערכת לשני חלקים עיקריים – צד לקוח (GUI) וצד שרת (PostgreSQL + pgAdmin) – ודאגתי שכל רכיב יתבצע עם הכלי המתאים ביותר לו.

1. שפת תכנות – Python
   <br>
את כל הקוד של המערכת כתבתי ב־Python. בחרתי בשפה הזו בגלל הפשטות שלה, התמיכה הרחבה במסדי נתונים, והיכולת לבנות ממשקים בצורה מהירה ונוחה. כל הלוגיקה שמאחורי הממשק, התקשורת עם המסד, וניהול הנתונים – כולם כתובים בפייתון.

2. ממשק גרפי – tkinter עם ttkbootstrap
  <br> 
לבניית הממשק הגרפי השתמשתי ב־tkinter, כי הוא מובנה בפייתון ונוח לעבודה. כדי להוסיף למראה יותר מודרני ויפה, שילבתי את הספרייה ttkbootstrap, שמוסיפה עיצוב משופר: כפתורים מעוגלים, צבעים מתקדמים, ואייקונים ברורים. כל מסך (לשחקנים, קבוצות, משחקים ושאילתות) נבנה כחלון עצמאי עם טופס מלא לניהול נתונים.

3. מסד נתונים – PostgreSQL
   <br>
בחרתי ב־PostgreSQL לניהול הנתונים של המערכת. הוא מסד נתונים רלציוני חזק שמאפשר לי להגדיר טבלאות, קשרים בין ישויות, פונקציות ופרוצדורות בצורה נוחה ומתקדמת. כל הנתונים של השחקנים, הקבוצות, המשחקים והטורנירים נשמרים שם.

4. ממשק ניהול מסד – pgAdmin (דרך Docker)

5. סביבה מקומית עם Docker

-----
### תמונות
![צילום מסך 2025-06-20 121844](https://github.com/user-attachments/assets/0439b9ae-d2aa-4633-a233-6470fa823a86)

![צילום מסך 2025-06-20 121950](https://github.com/user-attachments/assets/be95ffd9-f298-4326-93d6-8d111de69cff)

![צילום מסך 2025-06-20 122000](https://github.com/user-attachments/assets/5cb67a4d-bc35-4c2e-8920-019fbb2c22c6)

![צילום מסך 2025-06-20 122015](https://github.com/user-attachments/assets/f89f5d82-7a3f-46ca-be33-f1cf42322f62)

![צילום מסך 2025-06-20 122024](https://github.com/user-attachments/assets/c8aec00b-bb57-43f6-aece-c671f0dbce02)
