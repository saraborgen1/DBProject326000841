...
* `main_menu.py` – קובץ ראשי להפעלת הממשק
* `db_config.py` – הגדרות התחברות למסד הנתונים
* `player_crud.py` – ניהול טבלת שחקנים
* `team_crud.py` – ניהול טבלת קבוצות
* `game_crud.py` – ניהול משחקים
* `queries_and_procedures.py` – מסך הפעלת שאילתות ופונקציות
...
3. להריץ את הקובץ הראשי מהמסוף או IDE:
   `python main_menu.py`

פונקציונליות:

* **Manage Players** – מסך לניהול שחקנים: הצגה, הוספה, עדכון ומחיקה.
* **Manage Teams** – ניהול קבוצות לפי פרטים כמו שם, מדינה, צבע מדים, דירוג ועוד.
* **Manage Games** – ניהול משחקים כולל תאריך, מיקום, מזהה טורניר.
* **Queries & Procedures** – מסך להפעלת שאילתות, פונקציות ופרוצדורות.
  ...
  * פונקציות:
    ...
    * `get_all_players_points()` – מחזירה את סך הנקודות לשחקנים.
  * פרוצדורות:

    * `increase_experience_of_coaches()` – מעלה את ניסיון המאמנים.
    * ניתן להחליף או להוסיף פרוצדורות נוספות לפי הצורך.
